> ⚠ **Phase 1 (alpha) — pending Phase 2/3 work.**
> This document is ported from scalefinal-qa-assistant. Provider-specific instructions reference the future state where adapter framework exists.
> Currently you cannot follow this end-to-end (provider adapters in Phase 2). Read for vision; await Phase 2/3 release for actual install.
...
> Existing instance owner (Yaroslav for your product) — skip this file, use HOWTO.md.
...
This script will:
- Verify Python 3.9+, Node 18+
- Create `.venv` + install Python dependencies (`requirements.txt`)
- Install `qa-orchestra` plugin (foundation)
- Apply `qa-orchestra-fix.sh` (fix manifest + sparse-checkout)
- Install this plugin (extension layer)
...
**`_module_taxonomy.json` is critical for Product Map** — define 6-12 modules of your product (e.g. `auth`, `billing`, `users`, `inventory`, ...). Replace `{your-module-N}` placeholders. See your product source repo for working 12-module example.
