...
case "$cmd" in
  ...
  bug)
    ...
    if [[ -z "$id" || -z "$title" ]]; then
      ...
      echo "   Example: journal.sh bug TRD-13700 \"Bulk send fails\" staging \"1st cohort\"" >&2
      exit 1
    fi
    acquire_lock
    ensure_active
...
PY
    ...
    if [[ -f "$ACTIVE" ]]; then
      ...
      if [[ -n "$mission" && "$mission" != *"not set"* ]]; then
        today_lines+=("- $mission (in progress)")
      fi
    fi
    if [[ -f "$TODAY_FILE" ]]; then
      while IFS= read -r m; do
        today_lines+=("- $m ✅")
      done < <(python3 - "$TODAY_FILE" <<'PY'
import sys, pathlib, re
text = pathlib.Path(sys.argv[1]).read_text()
for m in re.findall(r"^## Session \d+ — (.+?) _\(saved", text, re.MULTILINE):
    print(m)
PY
)
    fi
