...
The two together: when TRD-X touches "client-mgmt module":
...
**Source of taxonomy:**
- [COMPANY] CRM real module structure (per CONTEXT.md modules list)
...
```json
{
  ...
  "recent_bugs": {
    "open_count": 5,
    "recent_examples": [
      {"id": "{TICKET_PREFIX}-XXXXX", "title": "Brand variables not updating in template", "status": "Open"},
      {"id": "{TICKET_PREFIX}-XXXXX", "title": "Bulk emails not sending to multi-brand selection", "status": "Done"}
    ],
  ...
  "schema_drift_notes": [
    {"note": "stage adds users.preferred_locale, release doesn't yet (rollout TRD-XXXXX)", "source": "knowledge_base/db_diff__stage_vs_release.md"}
  ]
}
```
...
## Bugs
- {TICKET_PREFIX}-XXXXX (no clear module tag) — keywords matched none. Triage manually.
...
🗺  Product Map — module: <client-mgmt>
   ...
   Recent bugs:    5 open, examples: {TICKET_PREFIX}-XXXXX, {TICKET_PREFIX}-XXXXX
   Allure:         47 cases, 92% pass rate (recent)
   Schema drift:   stage adds users.preferred_locale (rollout TRD-XXXXX)
...
| **Map file becomes huge** | Low | Medium | Currently estimate ~30-50KB for [COMPANY]. If grows past 200KB → split per-module files |
...
5. **Taxonomy seed for friend's brain** — does `templates/_module_taxonomy.json.tmpl` ship pre-populated or empty? Proposal: empty + README pointing to [COMPANY] example.
