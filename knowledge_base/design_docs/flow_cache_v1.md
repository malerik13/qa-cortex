...
|---|---|---:|---:|---:|
| {TICKET_PREFIX}-XXXXX (Forgot Password) | `browser_snapshot` | 17 | ~22K | **48%** |
| {TICKET_PREFIX}-XXXXX (Brand variables) | `browser_snapshot` | 7 | ~3-5K | ~10% (after token-economy fix) |
| {TICKET_PREFIX}-XXXXX (Brand variables) | `browser_evaluate` | 50 | ~24K | ~40% |
...
related_trd: [{TICKET_PREFIX}-XXXXX, {TICKET_PREFIX}-XXXXX]         # tickets this flow appeared in (recency hint)

selectors_strategy: data-test-first         # data-test-first | role-name-first | css-fallback
known_traps: [aaa-2fa-lockout, session-30min-expiry]   # IDs of known issues, see §6.4
...
## Preconditions
- <list of state requirements, e.g. "user not logged in", "stage env up", "test client {TICKET_PREFIX}-XXXXX exists">
...
## Refresh history
- 2026-05-06 — discovered ({TICKET_PREFIX}-XXXXX retest)
...
1. **`data-test` / `data-testid` / `data-cy`** — explicitly QA-stable attributes (when [COMPANY] frontend has them)
  ...
  "recipes": [
    {
      ...
      "related_trd": ["{TICKET_PREFIX}-XXXXX", "{TICKET_PREFIX}-XXXXX"],
      "playwright_spec": null
    },
    ...
  ],
  "by_trd": {
    "{TICKET_PREFIX}-XXXXX": ["auth.login.aaa", "comms.send-email-from-template"],
    ...
  },
  "by_area": {
...
When `start-ticket-test` skill activates on TRD-X:
...
```
🔍 Flow recipe lookup:
   Found 3 candidates for {TICKET_PREFIX}-XXXXX (Forgot Password area = auth):
...
5. **Session metadata** — TRD-IDs touched, role used, env, total tokens
...
```json
{
  ...
  "session-30min-expiry": {
    ...
    "source": "discovered {TICKET_PREFIX}-XXXXX"
  }
}
```
...
| Crosslink | `allure_case_id` field | TRD-ID + manual link to recipe |
...
Concept transfers fully. Friend's brain on Jira+TestRail stack would have its own `flows/` with recipes for *its* product, not [COMPANY]. Templates in `templates/flows/` need:
...
| **Recipes leak [COMPANY] IP** | Low | Medium | Recipes describe selectors + flows, not customer data. Same privacy posture as KB. |
| **Friend's brain tries to import [COMPANY] recipes** | Low | Low | Recipes per-stack; templates skeleton clean of [COMPANY]-specific |
...
2. **Recipe versioning when UI changes major version?** [COMPANY] v3.0 → v4.0 hypothetical: recipes become invalid. Strategy: tag recipe with `compatible_versions: [3.x]`, on v4.0 detection → mark all 3.x recipes as needing refresh.
...
4. **Cross-env templating depth?** A recipe for stage with `https://stage.[company].io/login` — how to make it work on release-ca? Via env var substitution. But what about role-specific flows that exist only on certain envs? Proposal: `env: [stage, release]` whitelist + skip on unsupported.
