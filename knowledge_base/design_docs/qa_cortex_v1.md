...
> **Author:** scalefinal-qa-assistant brain (with Yaroslav)
> **Created:** 2026-05-07
...
**qa-cortex** — open-source QA scaffold built around the same architecture as `scalefinal-qa-assistant`, but stack-agnostic and customizable. Default integrations: Jira + TestRail + Confluence + Slack + Playwright (with flow cache). Anyone can clone, configure for their stack, and have a working autonomous-on-routine + approval-gated-on-critical QA co-engineer in <1 hour.

**Vision:** Yaroslav's `scalefinal-qa-assistant` is a private *instance* of a *general framework*. Extracting that framework as `qa-cortex` lets:
...
**Scope:** real product, not template-rename. ~20-30 focused sessions, 4-8 weeks calendar time, in parallel with continuing scalefinal-qa-assistant operationalization.
...
`scalefinal-qa-assistant` is a working production-grade QA brain at v0.7.1:
...
- Если перейдёт на другую работу — заберёт architecture, оставит ScaleFinal-specific KB
- Forces clean separation, exposes what's actually generic vs ScaleFinal-isms

**Reason 2 — Friend's Jira/TestRail stack:**
- Already discussed: friend doesn't have YouTrack/Allure
- Currently no clean path: copy scalefinal-qa, manually swap MCP servers, hope skills don't break
...
20-30 sessions over 4-8 weeks, in parallel with scalefinal-qa-assistant operationalization. Real time investment.
...
4. **Same architecture as scalefinal-qa-assistant** — trust tiering, flow cache, product map, skills bridges
...
4. **Not Yaroslav's day job** — it's project alongside scalefinal-qa-assistant operationalization, не вместо
...
### 5.3 What changes vs scalefinal-qa-assistant

**Skills become provider-agnostic:**

```python
# Before (scalefinal-qa-assistant skills/start-ticket-test/SKILL.md):
mcp__plugin_scalefinal-qa-assistant_youtrack__get_ticket(ticket_id="TRD-XXXXX")
...
**ScaleFinal-specific KB content removed:**
...
- `flows/` — empty (no ScaleFinal recipes)

**ScaleFinal stays in scalefinal-qa-assistant:**
...
github.com/malerik13/scalefinal-qa-assistant     PRIVATE forever
   ├── Inherits from qa-cortex (manual sync, not git submodule initially)
   ├── ScaleFinal-specific MCPs (YouTrack, Allure)
   ├── ScaleFinal KB content
   ├── ScaleFinal flow recipes
   └── ScaleFinal config + secrets
```

**Sync mechanism:** Manual cherry-pick from qa-cortex → scalefinal-qa-assistant when generic improvements happen. Initially manual; possibly automated later via shared submodule pattern.

### 6.2 Why not Option B (template + overlay)

Considered: qa-cortex is template, scalefinal generates from it.

**Rejected because:**
- Adds complexity early (need template engine, overlay merge logic)
- ScaleFinal already has historical commits — rebasing onto template structure = git history surgery
...
Once created, qa-cortex repo becomes the **scaffold source of truth.** scalefinal-qa-assistant pulls generic improvements via cherry-pick.
...
**For scalefinal-qa-assistant** (private instance):
...
- Benefit: skills become identical between scalefinal and qa-cortex (sync becomes trivial)
...
| **YouTrack** | 🔒 via scalefinal | own MCP — stays in scalefinal-qa-assistant repo | Full |
...
| **Allure** | 🔒 via scalefinal | own MCP — stays in scalefinal | Full |
...
Playwright MCP — already universal, no provider abstraction needed. Same as scalefinal.
...
- Yaroslav loses no rights (still owns scalefinal-qa-assistant private fork)
...
├── INSTALL.md                 ← Detailed install steps (extends current scalefinal version)
...
│       ├── flow_cache_v1.md   ← Ported from scalefinal
│       ├── product_map_v1.md  ← Ported from scalefinal
...
## 11. Sync model (qa-cortex ↔ scalefinal-qa-assistant)

### 11.1 Initial state — fresh fork

When qa-cortex is first created:
- Files in qa-cortex are **clean version** of scalefinal files (ScaleFinal-isms removed)
- scalefinal-qa-assistant **does not yet** depend on qa-cortex — keeps its current code

### 11.2 Migration of scalefinal-qa-assistant onto qa-cortex foundation

**Phase A** (during qa-cortex Phase 2): build adapter layer in qa-cortex.
**Phase B** (after qa-cortex v0.5): migrate scalefinal to use qa-cortex framework:
- scalefinal keeps `mcp/youtrack/` (own MCP server) but it implements `qa_cortex.providers.TicketingProvider` interface
...
**Phase C** (steady state): scalefinal-qa-assistant = qa-cortex + ScaleFinal overlay (KB, recipes, providers).
...
qa-cortex (source of truth for generic)
   │
   │ (Yaroslav cherry-picks generic commits to scalefinal-qa-assistant)
   ▼
scalefinal-qa-assistant (instance with overlays)
```

Manual sync via `git cherry-pick`. Risk: drift. Mitigation: monthly «sync sweep» where qa-cortex changes audit which apply to scalefinal.

### 11.4 Future automation (not v1.0)

Once both are stable:
- Could use `git submodule` (qa-cortex as submodule в scalefinal)
- Could use shared package (qa-cortex publishes to PyPI, scalefinal pip installs)
...
5. Initial directory structure:
   ...
   ├── CLAUDE.md  (ported from scalefinal templates/CLAUDE.md.tmpl)
   ├── knowledge_base/
   │   ├── design_docs/
   │   │   ├── qa_cortex_v1.md  (this doc, ported)
   │   │   ├── flow_cache_v1.md (ported)
   │   │   └── product_map_v1.md (ported)
   │   ├── qa_persona.md (cleaned of ScaleFinal refs)
   ...
   └── CONTRIBUTING.md
   ```
6. Initial CLAUDE.md (ported from scalefinal, ScaleFinal-isms removed)
7. Trust tiering codified (copy-paste from scalefinal CLAUDE.md, generalize)
8. v0.0.1 tag
...
### 13.2 For users (runtime cost — same as scalefinal)

Trust tiering + flow cache + product map combine for ~77% reduction in per-session token cost compared to «no infrastructure» baseline. Per scalefinal projections: ~$5-10/month for active QA usage.

### 13.3 Maintenance time cost

- Initial Phase 1-3: ~20-30 sessions over 4-6 weeks
- Ongoing maintenance: ~5-10% of Yaroslav's brain-improvement time goes to qa-cortex sync, rest to scalefinal-specific work
...
|---|---|---|---|
| **Two products = double maintenance** | High | High | Discipline: qa-cortex is source of truth for generic, scalefinal sync via cherry-pick. Monthly audit. |
...
| **«ScaleFinal-isms» trap** (subtle ScaleFinal assumptions in code) | High initially | Medium | Diff review at extraction time; first install on different stack will surface remaining isms |
...
| **Yaroslav burnout** (this is a lot) | Medium | High | Realistic scope (4-8 weeks), hard limit on session count, parallel with scalefinal not instead |
...
- [ ] Repo public
- [ ] No accidental ScaleFinal leakage in commit history (git filter-branch if needed)
- [ ] License valid
...
- [ ] qa-cortex / scalefinal sync drift <1 month
...
1. **Does scalefinal-qa-assistant become qa-cortex + overlay or stay independent?**
...
5. **Multi-language support for personas?**
   ...
   - Possibly: English-only initially, RU stays in scalefinal-qa-assistant for now
...
| Just better docs in scalefinal | Low effort | Doesn't help friend's stack, doesn't enable 2nd machine flexibility |
...
- [ ] No — too big, focus on scalefinal only
...
- [ ] Approve doc → wait, return to scalefinal operationalization first
...
- **Forces clean architecture** — extracting generic from specific exposes ScaleFinal-isms, improves both products
- **Optional public release** — preserves option without forcing it
- **Phased + revertible** — Phase 1 is purely additive (new private repo), zero risk to scalefinal
...
- **scalefinal-qa-assistant** — Yaroslav's private instance of qa-cortex (or its predecessor, depending on migration phase)
...
- **Instance** — a configured user copy of qa-cortex (e.g. scalefinal-qa-assistant is an instance with YouTrack+Allure providers)
- **Overlay** — instance-specific files (KB content, recipes, custom MCPs) layered on qa-cortex base
- **Sync drift** — divergence between qa-cortex и scalefinal-qa-assistant when commits aren't synced
