...
> Yaroslav asked for help building this list. Below is a draft based on ISTQB foundations + your product context. Customize freely — anything you don't agree with, we kill or modify.
...
Tools: `/related <TICKET>-X`, `/explore <area>`, `get_linked_tickets`.
...
Tools: `/test <TICKET>-X`, `test-planning` skill.
...
| «Глянь <TICKET>-12345 в YouTrack и расскажи AC» | `youtrack.get_ticket(ticket_id="<TICKET>-12345")` |
...
Before any write, propose: «вот draft бага по <TICKET>-X, env=Release (because TRD is v3.0 demo target). Создавать?». Include explicit env choice + reasoning, не дефолт staging.
...
**Trigger to switch language mid-thought:** the surface, not the topic. If Yaroslav asks in Russian "напиши коммент в <TICKET>-X" — chat reply is Russian (acknowledging), but the **drafted comment is English**. Two messages: the chat ack in RU, the artefact in EN.
...
**Source:** 6 chat sessions on May 4, real testing of <TICKET>-11636 (Email builder Bulk actions). Filtered 65 user messages → 7 candidates → 6 useful.
...
## 11. Severity / Priority rubric — instance-specific
...
| Severity | When to use | Examples (instance-specific) |
|---|---|---|
...
| **Major** | Core feature broken for some users / one role / one env; functional flow blocked but workaround exists | Bulk Send Email Apply does nothing (<TICKET>-13752); export fails for Agent role; 2FA modal closes on backdrop click forcing reload |
...
### 11.2 your product floor rules (override default)
...
| Major | High | Bulk Send broken (<TICKET>-13752) — sprint must close it |
...
This rubric is calibrated against your product context. Review quarterly or after a sharp drift in 1st-cohort rate. If rate spikes, look for severity miscalibration (devs not fixing what they should because severity is wrong).
