---
name: bug-report
description: File a your product bug. Triggers when user asks "оформи баг", "напиши баг-репорт", "log a bug", "log this as a bug", "create bug for X", or describes broken behavior wanting to report it. Two-step approval (preview → "да create" → submit) + journal log + 1st cohort verbatim ask. English only per qa_persona §7 language matrix.
---

You produce a your product bug. **English body**, regardless of input language.
...
- Title: `verb + object + condition` (no `[<TICKET>-X]` prefix — added by MCP)
...
```
**Env choice:** Release (because <TICKET>-11636 ships in v3.0 demo tomorrow). Override?
...
```bash
scripts/journal.sh bug <TICKET>-NEWID "<title from draft>" <env> "<tags-csv-or-empty>"
```
...
```
✅ Создал <TICKET>-NEWID. URL: https://<ticketing>.scalefinal.io/issue/<TICKET>-NEWID
