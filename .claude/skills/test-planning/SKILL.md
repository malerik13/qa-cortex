---
name: test-planning
description: Build test scenarios from a YouTrack ticket's AC. Fetches AC + existing Allure cases via our MCPs, generates scenario list (4 categories — Happy/Negative/Boundary/Edge), surfaces top risks. Triggers when the user says "составь тест-план для TRD-X", "what should I test for TRD-X", "разбери AC TRD-X", "напиши сценарии для TRD-X", or names a ticket and asks specifically for test cases (NOT full lifecycle — for full pipeline use start-ticket-test).
---
...
If TRD-ID in message → use it. Else STOP, ask: «Какой тикет? Например {TICKET_PREFIX}-12345».

---

## Step 2 — Fetch AC + Allure (PARALLEL batch)

```
PARALLEL tool calls in one message:
- mcp__youtrack__get_ticket(ticket_id="{TICKET_PREFIX}-XXXXX")
- mcp__youtrack__get_comments(ticket_id="{TICKET_PREFIX}-XXXXX", max_results=20)
- mcp__allure__find_test_cases_by_issue(
    issue_id="{TICKET_PREFIX}-XXXXX",
...
- Existing Allure cases with scenario steps
...
**Rules:**
- Don't duplicate existing Allure coverage (Step 2 results) — focus on **gaps**
...
```markdown
# {TICKET_PREFIX}-XXXXX — Test scenarios

## Coverage matrix
| AC # | Existing Allure | New scenarios needed |
|------|-----------------|----------------------|
| 1    | case-2544       | (covered)            |
...
```
📋 Test scenarios — {TICKET_PREFIX}-XXXXX

Coverage: N AC → M new scenarios (K existing Allure cases reused)
...
- «создай Allure cases» → invoke Task(qa-orchestra:automation-writer) для генерации тестов + allure:create_test_case
...
3. **Don't duplicate existing Allure coverage** — Step 2 result is anti-duplication input.
...
- **Allure index stale** (no cases found but comments mention them) → suggest `python3 scripts/update-allure-index.py`, proceed without Allure context.
